extends SceneTree
## Eight Letters end-to-end: boots the REAL main.tscn (full UI tree), feeds real
## InputEventKey events through _unhandled_input, and asserts the whole loop:
## straight into level 1 -> typed word accepted -> live scores + pills ->
## duplicate/unavailable rejects -> target reached -> next level -> 8-letter
## instant win -> bonus-mode + how-to UI states -> game over reveal -> best
## saved -> PLAY AGAIN. Run: godot --headless --script res://tests/run8_e2e.gd
